                                        ...
                                        <div class="row align-items-end">
                                            <div class="col-lg-8">
                                                <div class="page-header-title">
                                                    ...
                                                    <div class="row">
                                                        <h4 class="p-5" style="padding: 0.81em !important; padding-left: 2em !important;">Doctor List</h4>
                                                        {{--<span>Add <code>Delete</code> & Update</span>--}}

                                                        <div class="form-group row pt-3 pl-5 pr-5 pull-right" style="display:flex">
                                                            <label class="control-label pt-2 mr-2" for="search">Search by name&nbsp:</label>
                                                            <div class="mr-2">
                                                                <input id="search" style="width:200px; height: auto;" class="form-control" name="search"> 
                                                            </div>
                                                            <button id="beginsearch" onclick="search()" class="btn btn-warning pl-2" >Search </button>
                                                            ...
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="text-center" width="200px">
                                                                        <a href="{{route('add.doctor')}}" id="add_mentor_model" class="btn btn-success btn-sm">
                                                            ...
                                                            @foreach ($doctor as $doc)
                                                                <tr class="mentor">
                                                                    ...
                                                                    <td class="text-center">
                                                                        <a href="{{ URL::to('/edit/doctor/'.$doc->doctors_id) }}" class="btn btn-warning btn-sm" >
                                                                            <i class="glyphicon glyphicon-pencil"></i>
                                                                        </a>
                                                                        <a onclick="Delete_user( {{ json_encode($doc) }} )" href="#" class="btn btn-danger btn-sm">
...
// Delete
    function delete_Mentor(){
        $('.err_toggle').addClass('hidden');
            ...
            var data = new FormData();
            data.append('doctors_id', $('input[name=del_id]').val());

            $.ajax({
                type : 'POST',
                url : '{{ asset('') }}delete/doctor',
                beforeSend: function(xhr){xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));},
                contentType: false,
                data:data,
                processData: false,
                success: function(data) {
                    ...
                    $("#del_msg_success").html("<span class='text-success'>Doctor Successfully Deleted :</span>");
                       updatelist();
                ...
                success: function(data) {
                    ...
                    for (val in data['data']) {  
                        $('#mentortable').append('<tr>'+
                            ...
                            '<td>'+ data['data'][val]["category"] +'</td>'+
                            '<td>'+ data['data'][val]["introduction"] +'</td>'+
                            '<td>'+                                    
                                '<a href="/edit/doctor/'+ data['data'][val]["doctors_id"] +'" class="btn btn-info btn-sm" >'+
...
{   
  ...
  //  var page = $(this).attr('href').split('page=')[1];
        ...
        $.ajax({
            type : 'POST',
            url : '{{ asset('') }}update/doctorlist?page={{$_GET["page"]}}',
            beforeSend: function(xhr){xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));},
            contentType: false,
            processData: false,
            success: function(data) {
