                                                            ...
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="text-center" width="200px">
                                                                        <a href="{{route('add.category')}}" id="add_mentor_model" class="btn btn-success btn-sm">
                                                            ...
                                                            @foreach ($category as $cat)
                                                                <tr class="mentor">
                                                                    ...
                                                                    <td class="text-center">
                                                                        <a href="{{ URL::to('/edit/category/'.$cat->cat_id) }}" class="btn btn-warning btn-sm" >
                                                                            <i class="glyphicon glyphicon-pencil"></i>
                                                                        </a>
                                                                        <a onclick="Delete_user( {{ json_encode($cat) }} )" href="#" class="btn btn-danger btn-sm">
        ...
        $('.err_toggle').addClass('hidden');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#del_msg_success").html("<span class='text-success'>Category Successfully Deleted :</span>");
                       updatelist();
...
{   
  ...
  //  var page = $(this).attr('href').split('page=')[1];
        ...
        $.ajax({
            type : 'POST',
            url : '{{ asset('') }}update/categorylist?page={{$_GET["page"]}}',
            beforeSend: function(xhr){xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));},
            contentType: false,
            processData: false,
            success: function(data) {
                ...
                for (val in data['data']) {  
                    $('#mentortable').append('<tr>'+
                        ...
                        '<td> Root </td>'+
                        '<td>'+
                            '<a href="/edit/category/'+ data['data'][val]["cat_id"] +'" class="btn btn-warning btn-sm" >'+
                                '<i class="glyphicon glyphicon-pencil"></i>'+
                            '</a>&nbsp'+
                            '</a>'+
