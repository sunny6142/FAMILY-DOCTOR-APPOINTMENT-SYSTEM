                                                            ...
                                                            @foreach ($request_mentor as $ment)
                                                                <tr class="mentor{{$ment->id}}">
                                                                    ...
                                                                    <td class="text-left">
                                                                        ...
                                                                        <a style="width:100px" onclick="viewdetil( {{ json_encode($ment) }} )" href="#" class="btn btn-info" >
                                                                            <i class="glyphicon glyphicon-eye-open"></i> Approve     
                                                                        </a>
                                                                        <a style="width:100px" onclick="Delete_user( {{ json_encode($ment) }} )" href="#" class="btn btn-danger btn-sm">
                                                                            <i class="glyphicon glyphicon-trash"></i> Deny
                                                                        </a>
                                                                    </td>
                                                                </tr>
                                                            @endforeach
...
{   
  ...
  //  var page = $(this).attr('href').split('page=')[1];
        ...
        $.ajax({
            type : 'POST',
            url : '/admin/update/approval?page={{$_GET["page"]}}',
            beforeSend: function(xhr){xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));},
            contentType: false,
            processData: false,
            success: function(data) {
                ...
                for (val in data['data']) {  
                    $('#mentortable').append('<tr>'+
                        ...
                        '<td>'+ data['data'][val]["mentoring_session_time_v"] +'</td>'+
